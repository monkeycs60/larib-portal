...
async function waitForTableToLoad(page) {
  ...
  // Wait for loading overlay to disappear (it contains "Loading..." text)
  try {
    await page.waitForSelector('text=Loading...', { state: 'hidden', timeout: 5000 });
  } catch (e) {
    // Loading overlay might not appear at all, which is fine
  }
...
test.describe('Structure & Display Tests', () => {
  test('should display correct table structure for regular users', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);

    // Vérifier les colonnes visibles pour les utilisateurs
    await expect(page.getByRole('columnheader', { name: /status/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /name/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /exam type/i })).toBeVisible();
    ...
    await expect(page.getByRole('columnheader', { name: /user tags/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /actions/i })).toBeVisible();

    // Vérifier que les colonnes admin ne sont PAS visibles
    await expect(page.getByRole('columnheader', { name: /diagnosis/i })).not.toBeVisible();
    await expect(page.getByRole('columnheader', { name: /difficulty/i })).not.toBeVisible();
  });

  test('should display correct table structure for admin users', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);

    // Vérifier les colonnes admin
    await expect(page.getByRole('columnheader', { name: /status/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /name/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /exam type/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /diagnosis/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /difficulty/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /created at/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /admin tags/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /actions/i })).toBeVisible();

    // Vérifier que les colonnes user ne sont PAS visibles
    await expect(page.getByRole('columnheader', { name: /first completion/i })).not.toBeVisible();
    await expect(page.getByRole('columnheader', { name: /attempts/i })).not.toBeVisible();
    await expect(page.getByRole('columnheader', { name: /level/i })).not.toBeVisible();
  });

  test('should display dates in relative format without translation keys', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
...
test.describe('Filter Tests', () => {
  ...
  test('should filter cases by exam type using multi-select', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
  ...
  test('should display all filter options for admin users', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
...
test.describe('Action Tests', () => {
  test('should navigate to case detail when clicking View', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
    ...
    await page.waitForURL(/\/en\/bestof-larib\/[a-z0-9-]+$/, { timeout: 10000 });
    expect(page.url()).toMatch(/\/en\/bestof-larib\/[a-z0-9-]+$/);
  });

  test('should navigate to new attempt when clicking Start new attempt', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);

    // Cliquer sur "Start new attempt"
    const startButton = page.getByRole('link', { name: /start new attempt/i }).first();
    await startButton.click();

    // Vérifier l'URL avec le paramètre newAttempt
    await page.waitForURL(/\/en\/bestof-larib\/[a-z0-9-]+\?newAttempt=1/, { timeout: 10000 });
    expect(page.url()).toMatch(/newAttempt=1/);
  });

  test('should open edit dialog when clicking Edit button as admin', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
  ...
  test('should show delete confirmation when clicking Delete as admin', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
    ...
    const confirmation = page.locator('[role="dialog"], [role="alertdialog"]').first();
...
test.describe('Internationalization Tests', () => {
  test('should display correct translations when switching language', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);

    // Vérifier texte en anglais
    await expect(page.getByRole('heading', { name: /clinical cases/i })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: /cas cliniques/i })).toBeVisible();
  });
});
...
test.describe('Permission & Authorization Tests', () => {
  test('should redirect to login when accessing bestof-larib without authentication', async ({ page }) => {
...
test.describe('Navigation & Header Buttons Tests', () => {
  test('should open Tags Manager when clicking Tags Manager button', async ({ page }) => {
    await loginAs(page, 'user');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
  ...
  test('should navigate to statistics page when clicking Statistics button as admin', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
  ...
  test('should open create case dialog when clicking Create Case as admin', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/en/bestof-larib');
    await waitForTableToLoad(page);
