...
export async function CongesPageContent({ locale, searchParams, adminView }: { locale: 'en' | 'fr'; searchParams: Record<string, string | string[] | undefined>; adminView: boolean }) {
  const sp = searchParams
  const session = await requireAuth()

  if (!canAccessApp(session.user, 'CONGES')) {
    redirect(applicationLink(locale, '/dashboard'))
  }

  if (adminView && !canAdminApp(session.user, 'CONGES')) {
    redirect(applicationLink(locale, '/conges'))
  }

  const showAdmin = adminView
  ...
  const [t, userDashboard, calendarData, adminDashboard] = await Promise.all([
    ...
    adminView ? getAdminLeaveDashboard(frenchHolidays) : Promise.resolve(null),
  ])
