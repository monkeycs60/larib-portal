...
export async function BestofPageContent({
	locale,
	searchParams,
	adminView,
}: {
	...
	if (adminView && !canAdminApp(session.user, 'BESTOF_LARIB')) {
	...
	const asDifficultyArray = (
		...
		const filtered = arr.filter((entry) =>
			(validDifficulties as readonly string[]).includes(entry)
		);
	...
	const statusFilter: 'PUBLISHED' | 'DRAFT' | undefined =
		isAdmin && rawStatus && ['PUBLISHED', 'DRAFT'].includes(rawStatus)
			...
			: undefined;
	const userProgressFilter: CaseListFilters['userProgress'] =
		!isAdmin && rawStatus
			? rawStatus === 'completed'
				? 'COMPLETED'
	...
	const filters: CaseListFilters = {
		...
		createdFrom:
			typeof sp?.dateFrom === 'string' && sp.dateFrom
		...
		createdTo:
			typeof sp?.dateTo === 'string' && sp.dateTo
				...
				: undefined,
		firstCompletedFrom:
			typeof sp?.firstCompletionFrom === 'string' && sp.firstCompletionFrom
				? new Date(sp.firstCompletionFrom)
				: undefined,
		firstCompletedTo:
		...
		myDifficulty:
			typeof sp?.myDifficulty === 'string' &&
			['BEGINNER', 'INTERMEDIATE', 'ADVANCED'].includes(sp.myDifficulty)
	...
	const sortField =
		typeof sp?.sort === 'string' ? (sp.sort as CaseListSortField) : undefined;
	const sortDirection =
		typeof sp?.dir === 'string' && (sp.dir === 'asc' || sp.dir === 'desc')
			? sp.dir
			: undefined;
	...
	const [
		...
		listDiseaseTags(),
		isAdmin
			? listAdminTags().then((rows) =>
					rows.map((row) => ({ id: row.id, name: row.name }))
			  )
			: Promise.resolve([]),
		isAdmin ? listAdminTags() : Promise.resolve([]),
		session?.user?.id
			? listUserTags(session.user.id).then((rows) =>
					rows.map((row) => ({ id: row.id, name: row.name }))
			  )
			: Promise.resolve([]),
	]);
