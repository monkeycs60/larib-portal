...
export default function FiltersBar({ examTypes, diseaseTags }: { examTypes: ExamType[]; diseaseTags: DiseaseTag[] }) {
  ...
  function applyFilters() {
    ...
    if (datePreset) next.set('datePreset', datePreset)
  ...
  function pushWith(partial: Partial<Record<string, string>>) {
    ...
    ;['q','status','examTypeId','diseaseTagId','difficulty','dateFrom','dateTo','datePreset'].forEach((k) => current.delete(k))
  ...
  function applyPreset(preset: string) {
    ...
    const startOfWeek = (d: Date) => { const c = new Date(d); const day = c.getDay() || 7; c.setDate(c.getDate() - (day - 1)); return startOfDay(c) } // Monday
    const endOfWeek = (d: Date) => { const c = startOfWeek(d); c.setDate(c.getDate() + 6); return endOfDay(c) }
    ...
    switch (preset) {
      case 'today':
        from = formatYYYYMMDD(now); to = formatYYYYMMDD(now); break
      case 'yesterday': {
        const y = new Date(now); y.setDate(now.getDate() - 1)
        from = formatYYYYMMDD(y); to = formatYYYYMMDD(y); break
      }
      case 'last7': {
        const s = new Date(now); s.setDate(now.getDate() - 6)
        from = formatYYYYMMDD(s); to = formatYYYYMMDD(now); break
      }
      case 'last30': {
      ...
      case 'thisWeek':
        from = formatYYYYMMDD(startOfWeek(now)); to = formatYYYYMMDD(endOfWeek(now)); break
      case 'lastWeek': {
        const lastWeekRef = new Date(now); lastWeekRef.setDate(now.getDate() - 7)
        from = formatYYYYMMDD(startOfWeek(lastWeekRef)); to = formatYYYYMMDD(endOfWeek(lastWeekRef)); break
      }
      case 'thisMonth':
        from = formatYYYYMMDD(startOfMonth(now)); to = formatYYYYMMDD(endOfMonth(now)); break
      case 'lastMonth': {
        const ref = new Date(now.getFullYear(), now.getMonth() - 1, 1)
        from = formatYYYYMMDD(startOfMonth(ref)); to = formatYYYYMMDD(endOfMonth(ref)); break
      }
      case '':
        from = ''; to = ''; break
      default:
  ...
  return (
    <div className="flex flex-wrap items-end gap-3">
      ...
      <div>
        <label className="block text-xs mb-1">{t('filters.dateRange')}</label>
        <Select
          value={datePreset}
          onChange={(e) => {
      ...
      {datePreset === 'custom' ? (
        <>
          <div>
            ...
            <Input type="date" value={dateFrom} onChange={(e) => { setDateFrom(e.target.value); pushWith({ dateFrom: e.target.value, datePreset: 'custom' }) }} />
          </div>
          <div>
            <label className="block text-xs mb-1">{t('filters.dateTo')}</label>
            <Input type="date" value={dateTo} onChange={(e) => { setDateTo(e.target.value); pushWith({ dateTo: e.target.value, datePreset: 'custom' }) }} />
          </div>
        </>
      ) : null}
