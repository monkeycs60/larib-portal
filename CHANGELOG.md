...
- Updated files:
  ...
  - `app/[locale]/conges/components/leave-calendar-client.tsx`
  - `app/[locale]/conges/components/calendar-skeleton.tsx`
...
- What it does: Prefetches the "Start new attempt" workspace before users click it, caches per-user case state/attempt queries, streams the clinical cases table via Suspense with a full-width skeleton, hydrates a client cache so returning from a case shows the previous list instantly, prioritises the navbar logo for LCP, and enforces HTTP compression for the app shell.
...
- Updated files:
  ...
  - `app/[locale]/bestof-larib/components/cases-table-cache-hydrator.tsx`
  - `app/[locale]/bestof-larib/components/cases-table-fallback.tsx`
  ...
  - `lib/bestof-cache-key.ts`
  - `lib/stores/bestof-cases-cache.ts`
